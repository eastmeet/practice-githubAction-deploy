...
#ps -ef | grep "practice-githubAction-deploy-0.0.1-SNAPSHOT.jar" | grep -v grep | awk '{print $2}' | xargs kill -9 2> /dev/null
#
...
#nohup java -jar build/libs/practice-githubAction-deploy-0.0.1-SNAPSHOT.jar --spring.profiles.active=dev > /dev/null 2>&1 &